...
public:
    ...
    void updateEmployee()
    {
        ...
        record("Updated Employee ID " + to_string(id) + " (" + name + "): " + hrComments);
    }
...
// Using map instead of vector because ID lookup needs to be fast
...
void changePassword(const string &user, int id, const string &role)
{
    vector<string> lines;
    ...
    {
        stringstream ss(ln);
        string fUser, fId, fHash, fRole, fChanged;
        getline(ss, fUser, '|');
        getline(ss, fId, '|');
        getline(ss, fHash, '|');
        getline(ss, fRole, '|');
        ...
        if (fUser == user && stoi(fId) == id && fRole == role)
        {
            // check old password
            if (fHash != encp(oldPass))
            {
                cout << "Hmm—that does not look right. Your current password is incorrect.\n";
                return;
            }

            // new one
            cout << "Great, that is you! Now enter your new password: ";
            string newPass;
            getline(cin, newPass);
            ...
            if (newPass != confirm)
            {
                cout << "Sorry, those passwords did not match. Please try again later.\n";
                return;
            }

            ln = fUser + "|" + fId + "|" + encp(newPass) + "|" + fRole + "|1";
            found = true;
            break;
        }
    }
    ...
    if (!found)
    {
        cout << "Sorry, we could not find your account information.\n";
        return;
    }
    ...
    cout << "All set! Your password has been updated successfully.\n";
}
...
void user(const string &role)
{
    ...
    record("Created new " + role + " ID " + to_string(id) + " (" + name + ")");
}
...
void deleteEmp(int id)
{
    ...
    record("Deleted Employee ID " + to_string(id) + " (" + empName + "). Comment: " + comment);
    employees.erase(it);
    save();
...
void adminPanel()
{
    ...
    do
    {
        ...
        case 5:
            return;
        default:
            cout << "Invalid choice. Try again plz.\n";
        }
    } while (true);
}

// HR menu and handles employee data management operations
void HR(const string &currentUser, int id)
{
    ...
    do
    {
        ...
        {
            changePassword(currentUser, id, "employee");
            break;
        }
...
// Employee Menu panel
void empPanel(const string &currentUser, int id)
{
